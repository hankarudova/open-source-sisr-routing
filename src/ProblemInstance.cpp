...
void ProblemInstance::print() const
{
    ...
    for (const Node &node : nodes)
    {
        std::cout << node.id << ": x: " << node.x << " y: " << node.y << " demand: "
                  ...
                  << " tw duration: " << node.service_length << " pickup sibling: " << node.pickup_sibling 
                  << " delivery sibling: " << node.delivery_sibling << std::endl;
    }
}
...
{
    DistanceType dx = nodes[node_a].x - nodes[node_b].x;
    DistanceType dy = nodes[node_a].y - nodes[node_b].y;
...
bool readProblemFilePDPTW(const std::string &file_path, ProblemInstance &instance)
{
    ...
    while (std::getline(file, line))
    {
        ...
        iss >> node.id >> node.x >> node.y >> node.demand >> node.service_start >> node.service_end >> node.service_length >> node.pickup_sibling >> node.delivery_sibling;
