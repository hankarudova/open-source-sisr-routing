...
// Class representing a PDPTW problem solver based on the algorithms from SISRs article
class Sisrs
{
private:
    ...
    // Fraction of iterations dedicated to fleet minimization
    float fleet_minimise_ratio = 0.1f;
    ...
    std::vector<int> weights = {4, 4, 2, 1, 1, 1, 1, 1, 1, 1};
    ...
    // Read config and setup parameters of the algorithm
    std::string config_path;
    void setupParametersFleet();
    void setupParametersDistance();
    ...
    bool recreate(DistanceType max_cost, int routes_cap, bool allow_absence);

    // simulated annealing algorithm (1)
    void minimiseDistance(int n_iterations);

    // Fleet minimization algorithm (4)
    void minimiseFleet(int n_iterations);
