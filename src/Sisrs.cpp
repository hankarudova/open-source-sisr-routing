...
bool Sisrs::recreate()
{
    ...
    double current_shaw = shaw*random_y(gen);
...
void Sisrs::setupParameters(const std::string &config_path)
{
    ...
    n_iter = iterations;
    problem.vehicles = config.value("n_vehicles", 2);
...
bool Sisrs::saveJson(const std::string &path, float duration)
{
    ...
    for (int i = 0; i < sol_best->routes.size(); ++i)
    {
        ...
        nlohmann::json vehicle = {
            ...
            {"MaxDistance", problem.nodes[problem.end_depot_index].service_end}
        };
...
nlohmann::json Sisrs::logParameters()
{
    ...
    algorithm_stats["Sortings"].push_back({"TW_WIDTH", weights[6]});
    algorithm_stats["Sortings"].push_back({"TW_START", weights[7]});
    algorithm_stats["Sortings"].push_back({"TW_END", weights[8]});
