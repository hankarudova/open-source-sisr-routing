                ...
                *sol_current = *sol_neighbor;
                ...
                if (sol_neighbor->total_score > sol_best->total_score)
                {
                    *sol_best = *sol_neighbor;
                    improvements.push_back({{"Iteration", i + 1}, 
                        {"Score", sol_best->total_score}, 
                        {"Distance", sol_best->total_cost}, 
                        {"Sort", last_sorting},
...
bool Sisrs::recreate()
{
    ...
    case SCORE_TO_DEMAND:
        last_sorting = "SCORE_TO_DEMAND";
        std::sort(sol_neighbor->absent_customers.begin(), sol_neighbor->absent_customers.end(), [this](int a, int b)
                  { return problem.nodes[a].score * problem.nodes[b].demand > problem.nodes[b].score * problem.nodes[a].demand; });
        break;
    ...
    // Line 3 - iterate over all unserved customers
    while (que.size() > 0)
    {
        int index = que.size() - 1;

        if (last_sorting != "RANDOM" && current_shaw > 0.0001f)
        {
            index = std::floor(que.size() * pow(random_y(gen), current_shaw));
        }
        ...
        DistanceType customer_service_length = problem.nodes[customer].service_length;
        ...
        for (int i = 0; i < sol_neighbor->routes.size(); ++i)
        {
            ...
            // Explore only the routes that customer fits into
            if (customer_service_length > max_dist_increase || problem.capacity < route.demand + problem.nodes[customer].demand - 1e-6)
            {
                continue;
            }
            ...
            for (int j = 0; j < route_size + 1; ++j)
            {
                ...
                DistanceType insert_here_cost = problem.distance_matrix[prev_customer][customer] + problem.distance_matrix[customer][next_customer] - problem.distance_matrix[prev_customer][next_customer] + customer_service_length;
...
bool Sisrs::saveJson(const std::string &path, float duration)
{
    ...
    for (int i = 0; i < sol_best->routes.size(); ++i)
    {
        ...
        nlohmann::json vehicle = {
            ...
            {"EndLocation", problem.end_depot_index},
        ...
        for (int j = -1; j <= route_length; ++j)
        {
            ...
            {
                stop["Location"] = problem.end_depot_index;
            }
            else
            {
...
nlohmann::json Sisrs::logParameters()
{
    ...
    algorithm_stats["Sortings"].push_back({"SCORE_TO_DEMAND", weights[6]});
