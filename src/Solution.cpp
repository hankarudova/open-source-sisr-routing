...
Route::Route()
    : cost(0), ruined(false), demands(), latest_arrivals(), earliest_departures(), customers()
{
}
...
void Route::updateRouteVariables(ProblemInstance &p, int pos_index_pickup, int pos_index_delivery)
{
    if (customers.empty())
    {
        return;
    ...
    int begin = 1;
    if (pos_index_pickup == 0 || pos_index_pickup == -1)
    {
        ...
        demands[0] = p.nodes[customers[0]].demand;
        begin = 1;
    }
    else
    {
        begin = pos_index_pickup;
    }
    ...
    for (int i = begin; i < customers_count; ++i)
    {
        demands[i] = demands[i - 1] + p.nodes[customers[i]].demand;
    }

    begin = customers_count - 2;
    if (pos_index_delivery == customers_count - 1 || pos_index_delivery == -1)
    {
        latest_arrivals[customers_count] = p.nodes[p.depot_index].service_end;
        latest_arrivals[customers_count - 1] = std::min(latest_arrivals[customers_count] - p.distance_matrix[customers[customers_count - 1]][p.depot_index] - p.nodes[customers[customers_count - 1]].service_length, p.nodes[customers[customers_count - 1]].service_end);

        begin = customers_count - 2;
    }
    else
    {
        begin = pos_index_delivery;
    }
...
{
    out << "cost: " << cost << " ruined: " << ruined << std::endl;
...
void Solution::createRoute(int pickup, int delivery)
{
    routes.emplace_back();
    Route &new_route = routes.back();
    int new_route_index = routes.size() - 1;

    new_route.customers.push_back(pickup);
    new_route.customers.push_back(delivery);
    new_route.demands.push_back(0);
    new_route.demands.push_back(0);
    ...
    new_route.earliest_departures.push_back(0);
    new_route.cost = problem.distance_matrix[problem.depot_index][pickup] + problem.distance_matrix[pickup][delivery] + problem.distance_matrix[delivery][problem.depot_index];
    new_route.updateRouteVariables(problem, -1, -1);

    customer_to_route[pickup] = new_route_index;
    customer_to_route[delivery] = new_route_index;
    routes_count += 1;

    total_cost += new_route.cost;
}

void Solution::insertIntoRoute(int route_index, int pickup, int pickup_position, int delivery, int delivery_position, DistanceType insertion_cost)
{
    auto &route = routes[route_index];

    route.customers.insert(route.customers.begin() + delivery_position, delivery);
    route.customers.insert(route.customers.begin() + pickup_position, pickup);
    route.earliest_departures.insert(route.earliest_departures.begin() + delivery_position, 0);
    route.earliest_departures.insert(route.earliest_departures.begin() + pickup_position, 0);
    route.latest_arrivals.insert(route.latest_arrivals.begin() + delivery_position, 0);
    route.latest_arrivals.insert(route.latest_arrivals.begin() + pickup_position, 0);
    route.cost += insertion_cost;
    total_cost += insertion_cost;
    route.demands.insert(route.demands.begin() + delivery_position, 0);
    route.demands.insert(route.demands.begin() + pickup_position, 0);
    customer_to_route[pickup] = route_index;
    customer_to_route[delivery] = route_index;
    route.updateRouteVariables(problem, pickup_position, delivery_position + 1);
}
...
void Solution::removeFromRoute(int route_index, const std::vector<int> &removed_customers)
{
    ...
    route.demands.resize(route.customers.size());

    route.updateRouteCost(problem);

    route.updateRouteVariables(problem, -1, -1);
    total_cost += route.cost;
}
...
void Solution::resetRoutes()
{
    ...
    // Recalculate all costs to avoid acumulation of floating point inaccuracy in distance by removing and inserting customers all over again
    total_cost = 0;
