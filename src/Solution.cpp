...
Route::Route()
    : cost(), score(0), demand(0), ruined(false)
{
}
...
void Route::updateRouteCost(ProblemInstance &p)
{
    ...
    {
        cost += p.distance_matrix[customers[j]][customers[j + 1]] + p.nodes[customers[j]].service_length;
    }
    cost += p.nodes[customers[customers.size() - 1]].service_length;
}
...
void Solution::printSolution(std::ostream &out)
{
    ...
    {
        out << "[" << i << "] ";
        out << "cost: " << routes[j].cost << " ruined: " << routes[j].ruined << " demand: " << routes[j].demand << " score: " << routes[j].score << " customers: ";
